                     ...
                     failure:(void (^)(NSError *))failure
{
    ...
    for (NSInteger i = 0; i < imageArr.count; i++) {
        ...
        NSURLSessionUploadTask* uploadTask = [UploadManager uploadTaskWithImage:imageArr[i] completion:^(NSURLResponse *response, id responseObject, NSError *error) {
            if (error) {
                KDLOG(@"第 %d 张图片上传失败: %@", (int)i + 1, error);
...
+ (NSURLSessionUploadTask*)uploadTaskWithImage:(UIImage *)image completion:(void (^)(NSURLResponse *response, id responseObject, NSError *error))completionBlock {
    ...
    @autoreleasepool {
        ...
        // 构造 NSURLRequest
        NSError* error = NULL;
        NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:@"http://app.1jzhw.com/api/Index/user_img" parameters:[JXJudgeStrObjc blockEncryption:mutableParams] constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
...
//            NSData *imageData = [image imageByScalingToWithSize:PIC_MAX_WIDTH];
            ...
            [formData appendPartWithFileData:imageData name:@"photo" fileName:@"upload.jpg" mimeType:@"image/jpeg"];
        } error:&error];
